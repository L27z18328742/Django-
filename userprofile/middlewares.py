...
class AuthMD(MiddlewareMixin):  # 验证登录
    white_list = ['/login/', '/admin/', '/error/','/register/']  # 白名单
    black_list = ['/blackend/', ]  # 黑名单
    ...
    def process_request(self, request):  # 请求之前
        ...
        print("_auth_user_id: ",_auth_user_id)
        ...
        elif request_url in self.white_list or request.session.get("_auth_user_id"):
            return None
        else:
